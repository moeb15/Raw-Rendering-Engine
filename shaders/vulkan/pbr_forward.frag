...
layout (location = 2) in flat uvec4 inTextures;
layout (location = 3) in vec4 inWorldPos;
layout (location = 4) in vec3 inViewPos;
layout (location = 5) in vec3 inNormal;
layout (location = 6) in flat uint inEmissive;
layout (location = 7) in flat float inCutoff;
layout (location = 8) in flat vec2 inRmFactor;
layout (location = 9) in flat vec4 inBaseColorFactor;
layout (location = 10) in vec4 inLightPos;
...
void main() 
{
	vec4 baseColor = texture(globalTextures[nonuniformEXT(inTextures.x)], inUV);
	if(baseColor.a <= inCutoff)
	{
		discard;
	}
	baseColor.rgba *= inBaseColorFactor.rgba;
	...
	vec4 rm = texture(globalTextures[nonuniformEXT(inTextures.z)], inUV);
	vec4 occ = texture(globalTextures[nonuniformEXT(inTextures.w)], inUV);
	vec4 emissive = texture(globalTextures[nonuniformEXT(inEmissive)], inUV);
	...
	float metalness = rm.g * inRmFactor.x;
	float roughness = rm.b * inRmFactor.y;
